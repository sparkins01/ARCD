...
namespace EventToAction
{
    class SynLogGen
    {
       ...
       // string dir = @"EventDatasets";
        ...
        public SynLogGen(SynParams p, string dir)
        {
            this.dir = dir;
            ...
            int numOfNonChainEvts = p.totalEvts - ((p.numOfChains * p.numOfEvtInChain) * p.repeatChains);
            ...
            if (finalEvts.Count == (chainEvts.Count * p.repeatChains) + nonChainEvts.Count)
            {
                ...
                fn = string.Format(dir + @"\Labelled_SyntheticEventLogs_{0}-{1}-{2}-{3}-{4}-{5}-{6}.csv",
                     p.numOfChains, p.numOfEvtInChain, p.numOfObjsInEvt, p.simBTWEvts, p.totalEvts, p.totalUniqueObjs, p.repeatChains);

                WriteToFile_Labels(finalEvts, fn);


            }
            else
                MessageBox.Show(string.Format("Error: {0} is not equal to {1} + {2}", finalEvts.Count, chainEvts.Count, nonChainEvts.Count));
